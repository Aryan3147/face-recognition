...
COOLDOWN_SECONDS = 10  # Log each person at most once per 10 seconds
...
class FaceRegistrationSystem:
    ...
    def send_to_make(self, session_log):
        """Send attendance records directly to Make webhook (blocking call)."""
        if not session_log:
            print("[Webhook] No records to send.")
            return

        csv_body = "\n".join(f"{name},{ts}" for name, ts in session_log)
        print(f"[Webhook] Sending payload:\n{csv_body}")
        ...
        try:
            ...
            print(f"[Webhook] HTTP {resp.status_code} - Response: {resp.text}")
        except requests.exceptions.ConnectionError as e:
            print(f"[Webhook] Connection error - check internet: {e}")
        except requests.exceptions.Timeout:
            print("[Webhook] Timed out - Make server did not respond in 15s")
        except Exception as e:
            print(f"[Webhook] Unexpected error: {type(e).__name__}: {e}")
    ...
    def run_detection(self):
        ...
        print("Press 'q' to quit | Press 'r' to register a new face")

        log_file = open("attendance.log", "a")
        last_logged = {}  # name -> last logged datetime
        session_log = []  # deduplicated records to send to Make
        ...
        while True:
            ...
            if face_rect is not None:
                ...
                if name:
                    ...
                    if last is None or (now - last).total_seconds() >= COOLDOWN_SECONDS:
                        ...
                        log_file.flush()
                        session_log.append((name, timestamp))
            ...
            if key == ord('q'):
                ...
                name_input = input("Enter name to register: ").strip()
                if name_input:
                    self.register_new_face(name_input)

        log_file.close()
        self.capture.release()
        cv2.destroyAllWindows()

        # Send directly (blocking) so it can't be killed before finishing
        self.send_to_make(session_log)
